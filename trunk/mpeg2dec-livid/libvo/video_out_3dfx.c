...
static int _3dfx_close (void *plugin);
static int _3dfx_setup (vo_output_video_attr_t *attr);
...
static int _3dfx_overlay       (overlay_buf_t *overlay_buf, uint16_t id);
static frame_t* _3dfx_allocate_image_buffer(uint32_t height, uint32_t width, uint32_t format);
static void _3dfx_free_image_buffer(frame_t* image);
...
#ifndef __OMS__
static vo_info_t _3dfx_vo_info = 
{
    "3dfx (/dev/3dfx)",
    "3dfx",
...
static int _3dfx_setup (vo_output_video_attr_t *attr)
{
	...
	if (memBase0 == (uint32_t *) 0xFFFFFFFF || memBase1 == (uint32_t *) 0xFFFFFFFF) {
		LOG (LOG_ERROR, "Couldn't map 3dfx memory areas: %p,%p,%d\n", 
...
static int _3dfx_draw_frame (frame_t *frame) 
{
	LOG (LOG_DEBUG, "video_out_3dfx: starting display_frame\n");

	// Put packed data onto page 2
	dump_yuv_planar ((uint32_t *)frame->base[0], (uint32_t *)frame->base[1], (uint32_t *)frame->base[2], vidpage2offset, vidwidth, vidheight);
...
static frame_t* _3dfx_allocate_image_buffer (int width, int height, uint32_t format)
{
    return libvo_common_alloc (width, height);
}


static void _3dfx_free_image_buffer (frame_t* frame)
{
