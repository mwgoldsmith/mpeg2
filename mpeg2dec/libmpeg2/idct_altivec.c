...
/*
 * C version of the altivec idct, requires compiler extensions
 *
 * The asm version is generated with:
 *
 * gcc-2.95 -fvec -I ../include -D__ALTIVEC__
 *	-O9 -fomit-frame-pointer -mregnames -S idct_altivec.c
 *
 * sed -e 's/"/\\"/g' -e 's/.Lfe/Lfe/g' idct_altivec.s |
 *	grep -v .file | grep -v gcc2_compiled | grep -v .ident |
 *	sed -e 's/^/"/g' -e 's/$/                                         /g' |
 *	expand | cut -c 1-72 | unexpand -a | sed -e 's/$/\\n\"/g'
 *
